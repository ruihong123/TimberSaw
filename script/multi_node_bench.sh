  ...
  n=1
    while [ $n -lt $nshard ]
  	do
  		...
  		ssh -o StrictHostKeyChecking=no ${compute_shard[n]} "screen -d -m cd /users/Ruihong && git clone --recurse-submodules $github_repo && cd TimberSaw/ && mkdir build &&  cd build && cmake -DCMAKE_BUILD_TYPE=Release .. && sudo apt-get install -y libnuma-dev  && make db_bench -j 32 && make Server -j 32" &
  		#
  		n=$((n+1))
  		sleep 1
  	done
