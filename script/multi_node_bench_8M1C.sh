...
#Set up compute nodes and run the benchmark at the same time
  ...
  do
    ...
    numactl --cpunodebind=all --localalloc ./db_bench --benchmarks=fillrandomshard,readrandomshard,readrandomshard --threads=16 --value_size=400 --block_size=8192 --num=312500 --bloom_bits=10 --cache_size=67108864 --IP=${memory_shard[n]} --tcpport=${communication_port[n]} --shardnum=${n} > result_1M8C_${n}.txt" &
    #
    n=$((n+1))
#    sleep 1
  done
